...
// Need to tweak these a bit for my section, these are the vanilla values:
bool ALLOW_EFFECTS = true;
int SOLID_TYPE = SOLID_BBOX;
// -Giegue
...
class CIns2Flame : ScriptBaseEntity, INS2BASE::ExplosiveBase
{
	...
	void FlameTouch( CBaseEntity@ pOther )
	{
		...
		if( ALLOW_EFFECTS && ( pOther is null || pOther.IsBSPModel() ) )
	...
	void RemoveEntity()
	{
		...
		if( ALLOW_EFFECTS ) SmokeMsg( self.GetOrigin() + Vector( 0, 0, Math.RandomLong( 8, 32 ) ), 32, 14, SPR_STEAM );
	...
	void AnimateThink()
	{
		...
		if( ALLOW_EFFECTS && self.pev.frame == 13 )
		{
		...
		if( ALLOW_EFFECTS && flDynLTime < g_Engine.time )
		{
			DynamicLight();
		...
		if( ALLOW_EFFECTS && flSprTTime < g_Engine.time )
		{
			SpriteTrail();
